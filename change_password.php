  ...
  if(isset($_POST['update'])){
    ...
    if(empty($errors)){
             ...
             if(sha1($_POST['old-password']) !== current_user()['password'] ){
               $session->msg('d', "Contraseña vieja incorrecta");
            ...
            $result = $db->query($sql);
                if($result && $db->affected_rows() === 1):
                  $session->logout();
                  $session->msg('s',"Ingrese con la nueva contraseña.");
                  redirect('index.php', false);
                else:
                  $session->msg('d',' No se pudo actualizar la clave');
                  redirect('change_password.php', false);
                endif;
    } else {
    ...
    <div class="text-center">
     ...
     <?php echo display_msg($msg); ?>
      <form method="post" action="change_password.php" class="clearfix">
        ...
        <div class="form-group">
              <label for="oldPassword" class="control-label">Vieja Contraseña</label>
              <input type="password" class="form-control" name="old-password" placeholder="Vieja Contraseña">
        </div>
