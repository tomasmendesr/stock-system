...
<?php
  if(isset($_POST['add_user'])){
    ...
    if(username_in_use($username,null)){
      $session->msg("d", "El username '{$username}' ya se encuentra en uso");
      redirect('add_user.php',false);
       ...
       if(empty($errors)){
           ...
           $password = sha1($password);
            ...
            if($db->query($query)){
              //sucess
              $session->msg('s',"Usuario creado éxito");
              redirect('add_user.php', false);
            } else {
              //failed
              $session->msg('d',' Falló la creación del usuario');
              redirect('add_user.php', false);
            }
       } else {
