...
class  Media {
  ...
  public function upload($file)
  {
    if(!$file || empty($file) || !is_array($file)):
      $this->errors[] = "Ningun archivo fue cargado.";
      return false;
    elseif($file['error'] != 0):
 ...
 public function process(){
    ...
    elseif(empty($this->fileName) || empty($this->fileTempPath)):
      $this->errors[] = "El directorio del archivo no esta disponible.";
      return false;
    elseif(!is_writable($this->productPath)):
      $this->errors[] = $this->productPath." debe poseer permisos de escritura!.";
      return false;
 ...
 /*--------------------------------------------------------------*/
  public function process_media(){
    ...
    if(move_uploaded_file($this->fileTempPath,$this->productPath.'/'.$this->fileName))
    {
      ...
      $this->errors[] = "La carga del archivo falló, posiblemente porque no se poseen los permisos necesarios en el directorio destino.";
      return false;
    }
 ...
 public function process_user($id){
    ...
    if(!is_writable($this->userPath)){
        $this->errors[] = $this->userPath." debe poseer permisos de escritura!.";
        return false;
      }
    if(!$id){
