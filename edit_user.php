...
//Update User basic info
  ...
  $username = remove_junk($db->escape($_POST['username']));
	if(username_in_use($username,$id)){
		$errors = "El username '{$username}' ya está en uso.";
	}
    else{
       ...
       $status   = remove_junk($db->escape($_POST['status']));
         ...
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            ...
            $session->msg('d',' Ups! Falló al actualizar el usuario');
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          }
    } else {
...
if(isset($_POST['update-pass'])) {
  ...
  if(empty($errors)){
     ...
     $h_pass   = sha1($password);
       ...
       $result = $db->query($sql);
        if($result && $db->affected_rows() === 1){
          $session->msg('s',"Contraseña actualizada ");
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        } else {
          $session->msg('d',' Ups! Falló al actualizar la contraseña');
          redirect('edit_user.php?id='.(int)$e_user['id'], false);
        }
  } else {
    $session->msg("d", $errors);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Actualizar datos de '<?php echo remove_junk(ucwords($e_user['name'])); ?>' 
        </strong>
       </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
          Cambiar clave de '<?php echo remove_junk(ucwords($e_user['name'])); ?>'
        </strong>
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          ...
          <div class="form-group clearfix">
                  <button type="submit" name="update-pass" class="btn btn-danger pull-right">Guardar</button>
          </div>
        </form>
      </div>
    </div>
